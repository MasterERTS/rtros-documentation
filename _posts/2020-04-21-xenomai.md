...
`https://www.kernel.org/`. We chose the 4.4.219 version, which is stable, recent, supported by Xenomai and maintained :
 
```shell
$ wget https://www.kernel.org/pub/linux/kernel/v4.x/linux-4.4.219.tar.gz
$ tar xf linux-4.4.219.tar.gz
```
...
You will have to choose the patch corresponding to the linux kernel you chose (4.4.x here) :

```shell
cd ~/rtos
wget https://xenomai.org/downloads/ipipe/v4.x/x86/ipipe-core-4.4.218-cip44-x86-23.patch
```
